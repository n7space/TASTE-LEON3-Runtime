...
static created_semaphores_count = 0;
static rtems_id hal_semaphore_ids[RT_MAX_HAL_SEMAPHORES];

static rtems_name generate_new_hal_semaphore_name()
{
...
{
    return (uint64_t)Timer_Apbctrl1_getCounterValue(&timer_2) << 32 | Timer_Apbctrl1_getCounterValue(&timer_1);
}
...
int32_t Hal_SemaphoreCreate(void)
{
    ...
    rtems_status_code status_code = rtems_semaphore_create(generate_new_hal_semaphore_name(),
...
bool Hal_SemaphoreObtain(int32_t id)
{
    ...
    do
    {
        status_code = rtems_semaphore_obtain(id, RTEMS_WAIT, RTEMS_NO_TIMEOUT);
    }
