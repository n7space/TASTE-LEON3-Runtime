...
/**
 * @brief                       Gathers monitoring information about every
 * sporadic/cyclic interface and update internal data structure that hold these
 * information.
 *
...
/**
 * @brief                       Returns structure containing information about
 * maximum execution time, minimum execution time, average execution time of a
 * given sporadic/cyclic interface
 *
 * @param[in] interface         Represents interface to obtain usage data
 * @param[out] usage_data       pointer to struct representing usage data of
 * given sporadic/cyclic interface, it is considered an output parameter
 *
 * @return                      Bool indicating whether the query about usage
 * data was successful
 */
...
/**
 * @brief                       Returns structure containing information about
 * current CPU usage in idle state (time when CPU was not used by any
 * sporadic/cyclic interface)
 *
 * @param[out] cpu_usage_data   pointer to struct representing idle usage data
 * regarding time when CPU was not used by any sporadic/cyclic interface
 *
 * @return                      Bool indicating whether the query about CPU
 * usage data was successful
 */
bool Monitor_GetIdleCPUUsageData(
    struct Monitor_CPUUsageData *const cpu_usage_data);

/**
 * @brief                       Returns maximum stack usage in bytes of a given
 * sporadic/cyclic interface.
 *
 * @param[in] interface         represents interface to obtain information about
 * stack usage
 *
 * @return                      represents maximum stack usage in bytes if >= 0,
 * and error otherwise.
 */
 ...
 *
 * @param[in] overflow_callback  pointer to function that implements message
...
/**
 * @brief                        Checks and returns current number of queued
 * items in sporadic interface queue
 *
 * @param[in] interface          represents interface to obtain information
 * about stack usage
 *
 * @return                       represents the number of queued items in
 * interface queue if >= 0, and error otherwise.
 */
int32_t Monitor_GetQueuedItemsCount(const enum interfaces_enum interface);

/**
 * @brief                        Checks and returns maximum reached number of
 * queued items in sporadic interface queue
 *
 * @param[in] interface          represents interface to obtain information
 * about maximum queue usage
 *
 * @return                       represents the maximum number of queued items
 * in interface queue if >= 0, and error otherwise.
 */
int32_t
 ...
 *
 * @param[out] activation_log                        pointer pointing to
 * beginning of cyclic buffer holding all activation entries
 * @param[out] out_latest_activation_entry_index     representing latest
 * activation log index
 * @param[out] out_size_of_activation_log            representing size of
 * activation log
 *
